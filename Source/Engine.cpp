...
namespace Autograd
{
    void backward(const std::shared_ptr<Value> &loss)
    {
        ...
        while (!stack.empty())
        {
            ...
            {
                // First time seeing curr_func, mark children as processed so that we process its parent step next time
                ...
                if (visited.find(curr_func) == visited.end())
                {
                    ...
                    {
                        // If input_val is Internal Node and the creator function has not been visited, push it onto the stack
